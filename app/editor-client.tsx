...
export default function EditorClient({ user }: { user: SessionUser }) {
  ...
  // الصورة تُضغط ثم تُرفع مرة واحدة وتُخزَّن منفصلة، ولا يُحفظ في المشروع
  // إلا مرجعها "img:<id>" — فيبقى ملف المشروع صغيرًا مهما كثرت الصفحات.
  const uploadImage = (file: File, callback: (ref: string) => void, small = false) => {
    const compressed = small ? compressImage(file, 700, 0.68) : compressImage(file);
    compressed
      .then(async dataUrl => {
        if (!projectId) { callback(dataUrl); return; }
        try {
          const { id } = await api<{ id: string }>(`/api/projects/${projectId}/images`, {
            method: 'POST',
            body: JSON.stringify({ data: dataUrl }),
          });
          callback(`${IMAGE_REF_PREFIX}${id}`);
        } catch (e) {
          saveMessage(e instanceof Error && e.message ? `تعذر رفع الصورة: ${e.message}` : 'تعذر رفع الصورة');
        }
      })
      .catch(() => saveMessage('تعذرت قراءة الصورة'));
  };
  ...
  return <div className="app-shell" dir="rtl">
    ...
    {showSettings && <SettingsModal settings={settings} onChange={updateSetting} onClose={() => setShowSettings(false)} onExport={exportProject} onImport={() => importRef.current?.click()} uploadImage={uploadImage} />}
